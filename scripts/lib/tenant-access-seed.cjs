...
const { RoleScope } = require('@prisma/client');

/**
 * Seed roles, menus, and tenant admin user on the active Prisma connection search_path.
 * @param {import('@prisma/client').PrismaClient | import('@prisma/client').Prisma.TransactionClient} prisma
 */
async function seedTenantAccess(prisma, options) {
  const adminEmail = (options.adminEmail || 'admin@wms.local').toLowerCase();
  const adminPassword = options.adminPassword || 'password123';
  const adminName = options.adminName || 'Tenant Admin';
  ...
  const inManifest = await upsertMenu(
    'INBOUND_MANIFEST_REVIEW',
    'Manifest Review',
    '/inbound/manifest-review',
    34,
    inboundParent.id,
  );
  ...
  const processActivity = await upsertMenu('PROCESS_ACTIVITY', 'Activity & billing', '/process/activity', 40, processParent.id);
  ...
  const integrationParent = await upsertMenu('INTEGRATION', 'Integrasi', '/integration/data-sync', 55);
  const integrationDataSync = await upsertMenu(
    'INTEGRATION_DATA_SYNC',
    'Sinkron data',
    '/integration/data-sync',
    56,
    integrationParent.id,
  );
  ...
  return { adminEmail, adminUserId: adminUser.id };
}

module.exports = { seedTenantAccess };
